  ...
  return (
    <>
      ...
      {open && mounted && createPortal(
        ...
        >
          <div
            className="w-full max-w-md rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-lg font-semibold">My Entry Pass</h3>
              <button
                className="px-2 py-1 rounded border border-[var(--border)] hover:bg-white/5 text-sm"
                onClick={() => setOpen(false)}
              >
                Close
              </button>
